locals {
  this_launch_template_id   = var.launch_configuration == "" && var.create_lc ? concat(aws_launch_template.this.*.id, [""])[0] : var.launch_configuration
  this_launch_template_name = var.launch_configuration == "" && var.create_lc ? concat(aws_launch_template.this.*.name, [""])[0] : ""
